...
def op1():
    ...
    plt.scatter(X_mds[:,0], X_mds[:,1], c=colormap[Y], alpha=0.5, edgecolors="black", linewidth=1)
    ...
    plt.title('MDS con n_components = 2')
...
def op2():
    ...
    # Plot 2D
    colormap = np.array(['#ff0000', '#0000ff'])
    plt.scatter(X_mds[:,0], X_mds[:,1], s=X_mds[:,2], c=colormap[Y], alpha=0.5, edgecolors="black", linewidth=1)
    pop_a = mpatches.Patch(color='#ff0000',label='Non Celiaco')
    pop_b = mpatches.Patch(color='#0000ff',label='Celiaco')
    plt.legend(handles=[pop_a,pop_b], loc='upper right')
    plt.title('MDS con n_components = 3')
    plt.xlabel("Coordinata 1")
    plt.ylabel("Coordinata 2")
    plt.show()
...
def op3():
    ...
    ax.scatter(X_mds[:,0], X_mds[:,1], X_mds[:,2], c=colormap[Y], marker='o', alpha=0.5, edgecolors="black", linewidth=1)
    pop_a = mpatches.Patch(color='#ff0000',label='Non Celiaco')
    pop_b = mpatches.Patch(color='#0000ff',label='Celiaco')
    plt.legend(handles=[pop_a,pop_b], loc='upper right')
    plt.title('MDS con n_components = 3')
